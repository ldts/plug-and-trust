...
extern int se050_i2c_read(uint8_t *buffer, int len);
extern int se050_i2c_write(uint8_t *buffer, int len);
extern int se050_i2c_init(void);
...
{
	return se050_i2c_read(buffer, len);
}

int phPalEse_i2c_write(void *foo, uint8_t *buffer, int len)
{
	return se050_i2c_write(buffer, len);
}
